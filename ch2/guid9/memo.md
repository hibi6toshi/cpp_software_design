...
まとめると、適切な依存構造を持つ正しいアーキテクチャを構築するには、抽象化の所有権に留意することが極めて重要となります。抽象化は実装に対する要件を表現するため、全ての依存関係が上位へ向くよう、アーキテクチャの上位で抽象化すべきです。

## まとめ
- 正しいアーキテクチャでは、下位の実装詳細が上位の抽象化に依存する点を肝に銘じておく。
- DIPに従い、アーキテクチャの上位で抽象化する。
- 抽象化を**所有**するのは、下位ではなく、アーキテクチャ上位とする。