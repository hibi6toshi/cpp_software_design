...
## 拡張性を優先んした時期尚早な設計
標準ライブラリは、拡張性に備えた設計の良いお手本です。
しかし、拡張性が重要だからといって、将来の拡張性を担保するだけのために、基底クラスやテンプレート全てのあらゆる実装詳細を、闇雲に対応させるベキではありません。
関心の分離を早計に行ってはいけませんしhtml、変更に備えた時期尚早な設計もすべきではありません。

コードの進化の見通しが不明瞭ならば、発生しない拡張を予測するよりも待った方が無難でしょう。
次に発生する拡張が将来の拡張を見通すヒントになるかもしれませんし、以降の拡張を容易にすべくリファクタリングする機会も生まれます。

そうしないと、ある拡張ばかりを優先し、他の拡張がずっと困難になるような事態が発生しかねません。
```
拡張に備えた設計とは、将来の変更に備えた設計の重要部分です。そのため、拡張が予測される機能には十分目を配り、拡張が良いになるような設計が肝要です。
```

## ガイドライン5の要約
- コードの拡張が容易になる設計を優先する。
- OCPに従い、拡張に対してはオープンに、改造に対してはクローズに、の状態を維持する。
- コードの追加には、基底クラス、テンプレート、関数オーバーロード、テンプレートの特殊化を用いるよう設計する。
